...
mcmc.ests <- read.csv(file.path(mcmc.dir, paste('ASAP_summary_Run',run.no,'.MCMC.csv',sep='')))
...
median.annual.ests <- mcmc.ests[,c('SSB','Recr','Freport')]
  colnames(median.annual.ests) <-   c('SSB', 'Rect', 'F')
...
terminal.yr.ests <- data.frame(matrix(NA,3,3)) 
  rownames(terminal.yr.ests) <- c('SSB.mt', 'Rect.thous', 'Avg.F')
...
{
  terminal.yr.ests['SSB.mt',  ]   <- mcmc.ests[       as.character(lyr),c('SSB','SSB_95_lo','SSB_95_hi')]
  terminal.yr.ests['Avg.F',   ]   <- round(mcmc.ests[ as.character(lyr),c('Freport','Freport_95_lo','Freport_95_hi')], 3)
  terminal.yr.ests['Rect.thous',] <- mcmc.ests[       as.character(lyr),c('Recr','Recr_95_lo','Recr_95_hi')]
}

if(estimate.type == 'point.est')
{
  terminal.yr.ests['SSB.mt',  ] <- c( annual.ests[as.character(lyr),'SSB'],         mcmc.ests[    as.character(lyr),c('SSB_95_lo','SSB_95_hi')] )       
  terminal.yr.ests['Avg.F',   ] <- c( annual.ests[as.character(lyr),'F'],           round(mcmc.ests[as.character(lyr),c('Freport_95_lo','Freport_95_hi')], 3) )           
  terminal.yr.ests['Rect.thous',] <- c( annual.ests[as.character(lyr),'Rect'],      mcmc.ests[    as.character(lyr),c('Recr_95_lo','Recr_95_hi')] )
}
# if(estimate.type == 'median')
# {
...
terminal.yr.ests.formatted <- terminal.yr.ests
  rownames(terminal.yr.ests.formatted) <- c('Spawning stock biomass (mt)', 'Recruitment (000s)', 'Average F (ages 6-9)')
